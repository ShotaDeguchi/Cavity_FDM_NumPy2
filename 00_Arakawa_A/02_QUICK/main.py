...
grid:
    collocated grid (Arakawa A-type grid, Arakawa&Lamb 1977)
********************************************************************************
"""
...
def main(args):
    ...
    v = np.zeros_like(u) + 1e-3
    p = np.zeros_like(u) + 1e-3
    b = np.zeros_like(u) + 1e-3
    ...
    while u_res > u_tol:
        ...
        # convection
        u_u_x = u_old[2:-2, 2:-2] * (- u_old[2:-2, 4:] + 10. * u_old[2:-2, 3:-1] - 10. * u_old[2:-2, 1:-3] + u_old[2:-2, :-4]) / (16. * dx) \
                + np.abs(u_old[2:-2, 2:-2]) * (u_old[2:-2, 4:] - 4. * u_old[2:-2, 3:-1] + 6. * u_old[2:-2, 2:-2] - 4. * u_old[2:-2, 1:-3] + u_old[2:-2, :-4]) / (16. * dx)
        v_u_y = v_old[2:-2, 2:-2] * (- u_old[4:, 2:-2] + 10. * u_old[3:-1, 2:-2] - 10. * u_old[1:-3, 2:-2] + u_old[:-4, 2:-2]) / (16. * dy) \
                + np.abs(v_old[2:-2, 2:-2]) * (u_old[4:, 2:-2] - 4. * u_old[3:-1, 2:-2] + 6. * u_old[2:-2, 2:-2] - 4. * u_old[1:-3, 2:-2] + u_old[:-4, 2:-2]) / (16. * dy)
        u_v_x = u_old[2:-2, 2:-2] * (- v_old[2:-2, 4:] + 10. * v_old[2:-2, 3:-1] - 10. * v_old[2:-2, 1:-3] + v_old[2:-2, :-4]) / (16. * dx) \
                + np.abs(u_old[2:-2, 2:-2]) * (v_old[2:-2, 4:] - 4. * v_old[2:-2, 3:-1] + 6. * v_old[2:-2, 2:-2] - 4. * v_old[2:-2, 1:-3] + v_old[2:-2, :-4]) / (16. * dx)
        v_v_y = v_old[2:-2, 2:-2] * (- v_old[4:, 2:-2] + 10. * v_old[3:-1, 2:-2] - 10. * v_old[1:-3, 2:-2] + v_old[:-4, 2:-2]) / (16. * dy) \
                + np.abs(v_old[2:-2, 2:-2]) * (v_old[4:, 2:-2] - 4. * v_old[3:-1, 2:-2] + 6. * v_old[2:-2, 2:-2] - 4. * v_old[1:-3, 2:-2] + v_old[:-4, 2:-2]) / (16. * dy)
        conv_u = u_u_x + v_u_y
        conv_v = u_v_x + v_v_y
        ...
        for it in range(0, it_max+1):
            ...
            # boundary condition (long stencil shifts the boundary condition)
            p[1,  :] = p[2,  :]   # South (be careful with meshgrid)
            p[-2, :] = p[-3, :]   # North
            p[:,  1] = p[:,  2]   # West
            p[:, -2] = p[:, -3]   # East
            # p[1, 1] = 0.   # bottom left corner
            p[2, 2] = 0.   # bottom left corner
        ...
        if n % 1000 == 0:
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X[2:-2, 2:-2], Y[2:-2, 2:-2], div_u_hat, cmap="coolwarm", levels=levels, extend="both")
            ...
            # divergence & vorticity (only consider the interior points)
            u_x = (u[2:-2, 3:-1] - u[2:-2, 1:-3]) / (2. * dx)
            u_y = (u[3:-1, 2:-2] - u[1:-3, 2:-2]) / (2. * dy)
            v_x = (v[2:-2, 3:-1] - v[2:-2, 1:-3]) / (2. * dx)
            v_y = (v[3:-1, 2:-2] - v[1:-3, 2:-2]) / (2. * dy)
            div_u = u_x + v_y
            vor_u = v_x - u_y
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X[2:-2, 2:-2], Y[2:-2, 2:-2], div_u, cmap="coolwarm", levels=levels, extend="both")
            plt.colorbar(ticks=ticks, extend="both", label=r"$\nabla \cdot \mathbf{u}$")
            plt.xlim(0., Lx)
            plt.ylim(0., Ly)
            plt.xlabel(r"$x$")
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X[2:-2, 2:-2], Y[2:-2, 2:-2], vor_u, cmap="turbo", levels=levels, extend="both")
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X, Y, p - np.mean(p), cmap="turbo", levels=levels, extend="both")
