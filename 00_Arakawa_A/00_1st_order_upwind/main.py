...
def main(args):
    ...
    while u_res > u_tol:
        ...
        u_u_x = u_old[1:-1, 1:-1] * u_x - abs(u_old[1:-1, 1:-1]) * dx / 2. * u_xx
        v_u_y = v_old[1:-1, 1:-1] * u_y - abs(v_old[1:-1, 1:-1]) * dy / 2. * u_yy
        u_v_x = u_old[1:-1, 1:-1] * v_x - abs(u_old[1:-1, 1:-1]) * dx / 2. * v_xx
        v_v_y = v_old[1:-1, 1:-1] * v_y - abs(v_old[1:-1, 1:-1]) * dy / 2. * v_yy
        conv_u = u_u_x + v_u_y
        conv_v = u_v_x + v_v_y
        ...
        # source term for PPE
        u_hat_x = (u_hat[1:-1, 2:] - u_hat[1:-1, :-2]) / (2. * dx)
        v_hat_y = (v_hat[2:, 1:-1] - v_hat[:-2, 1:-1]) / (2. * dy)
        ...
        if n % 1000 == 0:
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X[1:-1, 1:-1], Y[1:-1, 1:-1], div_u_hat, cmap="coolwarm", levels=levels, extend="both")
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X[1:-1, 1:-1], Y[1:-1, 1:-1], div_u, cmap="coolwarm", levels=levels, extend="both")
            plt.colorbar(ticks=ticks, extend="both", label=r"$\nabla \cdot \mathbf{u}$")
            plt.xlim(0., Lx)
            plt.ylim(0., Ly)
            plt.xlabel(r"$x$")
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X[1:-1, 1:-1], Y[1:-1, 1:-1], vor_u, cmap="turbo", levels=levels, extend="both")
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X, Y, p, cmap="turbo", levels=levels, extend="both")
            plt.colorbar(ticks=ticks, extend="both", label=r"$p$")
            plt.xlim(0., Lx)
            plt.ylim(0., Ly)
            plt.xlabel(r"$x$")
            plt.ylabel(r"$y$")
            plt.title(f"Pressure (raw, $\mathrm{{Re}}={Re}, t={t:.2f}$)")
            plt.tight_layout()
            plt.savefig(os.path.join(dir_res, f"pressure_raw.png"), dpi=300)
            plt.close()
