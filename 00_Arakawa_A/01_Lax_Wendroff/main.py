...
def main(args):
    ...
    while u_res > u_tol:
        ...
        # convection
        u_u_x = u_old[1:-1, 1:-1] * u_x - u_old[1:-1, 1:-1]**2 * dt / 2. * u_xx
        v_u_y = v_old[1:-1, 1:-1] * u_y - v_old[1:-1, 1:-1]**2 * dt / 2. * u_yy
        u_v_x = u_old[1:-1, 1:-1] * v_x - u_old[1:-1, 1:-1]**2 * dt / 2. * v_xx
        v_v_y = v_old[1:-1, 1:-1] * v_y - v_old[1:-1, 1:-1]**2 * dt / 2. * v_yy
        conv_u = u_u_x + v_u_y
        conv_v = u_v_x + v_v_y
