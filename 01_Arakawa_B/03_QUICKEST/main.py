...
space:
    convection:        QUICKEST (Leaonard 1979)
...
def get_convection(u, v, dt, dx, dy, beta=None):
    u_u_x = u[2:-2, 2:-2] * (- u[2:-2, 4:] + 8. * u[2:-2, 3:-1] - 8. * u[2:-2, 1:-3] + u[2:-2, :-4]) / (12. * dx) \
            + np.abs(u[2:-2, 2:-2]) * (u[2:-2, 4:] - 4. * u[2:-2, 3:-1] + 6. * u[2:-2, 2:-2] - 4. * u[2:-2, 1:-3] + u[2:-2, :-4]) / (12. * dx) \
            - dt**1 * u[2:-2, 2:-2]**2 / 2. * (u[2:-2, 3:-1] - 2. * u[2:-2, 2:-2] + u[2:-2, 1:-3]) / dx**2 \
            + dt**2 * u[2:-2, 2:-2]**3 / 6. * (u[2:-2, 4:] - 2. * u[2:-2, 3:-1] + 2. * u[2:-2, 1:-3] - u[2:-2, :-4]) / (2. * dx**2)
    v_u_y = v[2:-2, 2:-2] * (- u[4:, 2:-2] + 8. * u[3:-1, 2:-2] - 8. * u[1:-3, 2:-2] + u[:-4, 2:-2]) / (12. * dy) \
            + np.abs(v[2:-2, 2:-2]) * (u[4:, 2:-2] - 4. * u[3:-1, 2:-2] + 6. * u[2:-2, 2:-2] - 4. * u[1:-3, 2:-2] + u[:-4, 2:-2]) / (12. * dy) \
            - dt**1 * v[2:-2, 2:-2]**2 / 2. * (u[3:-1, 2:-2] - 2. * u[2:-2, 2:-2] + u[1:-3, 2:-2]) / dy**2 \
            + dt**2 * v[2:-2, 2:-2]**3 / 6. * (u[4:, 2:-2] - 2. * u[3:-1, 2:-2] + 2. * u[1:-3, 2:-2] - u[:-4, 2:-2]) / (2. * dy**2)
    u_v_x = u[2:-2, 2:-2] * (- v[2:-2, 4:] + 8. * v[2:-2, 3:-1] - 8. * v[2:-2, 1:-3] + v[2:-2, :-4]) / (12. * dx) \
            + np.abs(u[2:-2, 2:-2]) * (v[2:-2, 4:] - 4. * v[2:-2, 3:-1] + 6. * v[2:-2, 2:-2] - 4. * v[2:-2, 1:-3] + v[2:-2, :-4]) / (12. * dx) \
            - dt**1 * u[2:-2, 2:-2]**2 / 2. * (v[2:-2, 3:-1] - 2. * v[2:-2, 2:-2] + v[2:-2, 1:-3]) / dx**2 \
            + dt**2 * u[2:-2, 2:-2]**3 / 6. * (v[2:-2, 4:] - 2. * v[2:-2, 3:-1] + 2. * v[2:-2, 1:-3] - v[2:-2, :-4]) / (2. * dx**2)
    v_v_y = v[2:-2, 2:-2] * (- v[4:, 2:-2] + 8. * v[3:-1, 2:-2] - 8. * v[1:-3, 2:-2] + v[:-4, 2:-2]) / (12. * dy) \
            + np.abs(v[2:-2, 2:-2]) * (v[4:, 2:-2] - 4. * v[3:-1, 2:-2] + 6. * v[2:-2, 2:-2] - 4. * v[1:-3, 2:-2] + v[:-4, 2:-2]) / (12. * dy) \
            - dt**1 * v[2:-2, 2:-2]**2 / 2. * (v[3:-1, 2:-2] - 2. * v[2:-2, 2:-2] + v[1:-3, 2:-2]) / dy**2 \
            + dt**2 * v[2:-2, 2:-2]**3 / 6. * (v[4:, 2:-2] - 2. * v[3:-1, 2:-2] + 2. * v[1:-3, 2:-2] - v[:-4, 2:-2]) / (2. * dy**2)
    conv_u = u_u_x + v_u_y
    conv_v = u_v_x + v_v_y
...
def main():
    ...
    while u_res > u_tol:
        ...
        # convection
        conv_u, conv_v = get_convection(u_old, v_old, dt, dx, dy, beta)
