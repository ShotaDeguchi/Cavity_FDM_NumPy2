...
def main(args):
    ...
    nu = 1. / Re
    eta = (nu**3 / abs(np.mean(u)))**.25

    # compute Kolmogorov velocity scale
    u_tau = (nu * abs(np.mean(u)) / eta)**.5

    # compute Kolmogorov time scale
    tau_eta = (nu / abs(np.mean(u)))**.5

    # add Smagorinsky model if necessary
    add_Smagorinsky = False
    ...
    while u_res > u_tol:
        ...
        if args.solver == "CG":
            # solve PPE with conjugate gradient method
            # get A (coefficient matrix)
            A = np.zeros(shape=((Ny-1) * (Nx-1), (Ny-1) * (Nx-1)))
            ...
            p = p.reshape(-1)
            b = b.reshape(-1)
            p, log = Conjugate_Gradient(
                A=A, b=b, x=p, 
                tol=p_tol, priori_check=True
            )
            p = p.reshape((Ny-1, Nx-1))
