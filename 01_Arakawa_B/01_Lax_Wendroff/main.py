...
def main():
    ...
    while u_res > args.u_tol:
        ...
        # convection 
        u_u_x = u_old[1:-1, 1:-1] * u_x - beta * u_old[1:-1, 1:-1]**2 * dt / 2. * u_xx
        v_u_y = v_old[1:-1, 1:-1] * u_y - beta * v_old[1:-1, 1:-1]**2 * dt / 2. * u_yy
        u_v_x = u_old[1:-1, 1:-1] * v_x - beta * u_old[1:-1, 1:-1]**2 * dt / 2. * v_xx
        v_v_y = v_old[1:-1, 1:-1] * v_y - beta * v_old[1:-1, 1:-1]**2 * dt / 2. * v_yy
        conv_u = u_u_x + v_u_y
        conv_v = u_v_x + v_v_y
        ...
        u[0,  :] = 0.   # South (be careful with meshgrid)
        u[-1, :] = 1.   # North
        u[:,  0] = 0.   # West
        u[:, -1] = 0.   # East
