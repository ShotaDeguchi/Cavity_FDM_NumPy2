...
def get_diffusion(u, v, dx, dy, nu, Smagorinsky=False, const=.173):
    ...
    if not Smagorinsky:
        ...
        """
        const = .173   # Lilly 1966 for homogeneous isotropic turbulence
        const = .1     # Deardoff 1970
        """
        delta = dx

        # strain rate tensor
        Ny, Nx = u.shape
        S = np.zeros(shape=(Ny, Nx, 2, 2))
        du_dx = (u[2:-2, 3:-1] - u[2:-2, 1:-3]) / (2. * dx)
        du_dy = (u[3:-1, 2:-2] - u[1:-3, 2:-2]) / (2. * dy)
        dv_dx = (v[2:-2, 3:-1] - v[2:-2, 1:-3]) / (2. * dx)
        dv_dy = (v[3:-1, 2:-2] - v[1:-3, 2:-2]) / (2. * dy)
        S[2:-2, 2:-2, 0, 0] = du_dx
        S[2:-2, 2:-2, 0, 1] = 1. / 2. * (du_dy + dv_dx)
        S[2:-2, 2:-2, 1, 0] = 1. / 2. * (dv_dx + du_dy)
        S[2:-2, 2:-2, 1, 1] = dv_dy
        # magnitude of strain rate tensor
        S_mag = np.sqrt(2. * np.sum(S**2, axis=(2, 3)))
        nu_t = (const * delta)**2 * S_mag
        print(f"   >>> Smagorinsky model: mean = {np.mean(nu_t):.6e}, max = {np.max(nu_t):.6e}")
        print(f"   >>> effective viscosity: mean = {np.mean(nu + nu_t):.6e}, max = {np.max(nu + nu_t):.6e}")
        diff_u = (nu + nu_t[2:-2, 2:-2]) * lap_u
        diff_v = (nu + nu_t[2:-2, 2:-2]) * lap_v
        return diff_u, diff_v
...
def main():
    ...
    while u_res > u_tol:
        ...
        # diffusion
        diff_u, diff_v = get_diffusion(u_old, v_old, dx, dy, k, Smagorinsky=True, const=.1)
