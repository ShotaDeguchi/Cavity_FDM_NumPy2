...
space:
    convection:        QUICK (Leaonard 1979)
...
def get_convection(u, v, dx, dy, beta):
    u_u_x = u[2:-2, 2:-2] * (- u[2:-2, 4:] + 10. * u[2:-2, 3:-1] - 10. * u[2:-2, 1:-3] + u[2:-2, :-4]) / (16. * dx) \
            + np.abs(u[2:-2, 2:-2]) * (u[2:-2, 4:] - 4. * u[2:-2, 3:-1] + 6. * u[2:-2, 2:-2] - 4. * u[2:-2, 1:-3] + u[2:-2, :-4]) / (16. * dx)
    v_u_y = v[2:-2, 2:-2] * (- u[4:, 2:-2] + 10. * u[3:-1, 2:-2] - 10. * u[1:-3, 2:-2] + u[:-4, 2:-2]) / (16. * dy) \
            + np.abs(v[2:-2, 2:-2]) * (u[4:, 2:-2] - 4. * u[3:-1, 2:-2] + 6. * u[2:-2, 2:-2] - 4. * u[1:-3, 2:-2] + u[:-4, 2:-2]) / (16. * dy)
    u_v_x = u[2:-2, 2:-2] * (- v[2:-2, 4:] + 10. * v[2:-2, 3:-1] - 10. * v[2:-2, 1:-3] + v[2:-2, :-4]) / (16. * dx) \
            + np.abs(u[2:-2, 2:-2]) * (v[2:-2, 4:] - 4. * v[2:-2, 3:-1] + 6. * v[2:-2, 2:-2] - 4. * v[2:-2, 1:-3] + v[2:-2, :-4]) / (16. * dx)
    v_v_y = v[2:-2, 2:-2] * (- v[4:, 2:-2] + 10. * v[3:-1, 2:-2] - 10. * v[1:-3, 2:-2] + v[:-4, 2:-2]) / (16. * dy) \
            + np.abs(v[2:-2, 2:-2]) * (v[4:, 2:-2] - 4. * v[3:-1, 2:-2] + 6. * v[2:-2, 2:-2] - 4. * v[1:-3, 2:-2] + v[:-4, 2:-2]) / (16. * dy)
    conv_u = u_u_x + v_u_y
    conv_v = u_v_x + v_v_y
