...
def main(args):
    ...
    nu = 1. / Re     # diffusion rate

    dt1 = h / U                 # CFL
    dt2 = 1. / 2. * h**2 / nu   # diffusion
    ...
    eta = Re**(-3. / 4.)
    print(f"Kolmogorov length scale: {eta:.2e}")
    print(f"Grid spacing: {h:.2e}")
    add_Smagorinsky = True
    ...
    while u_res > u_tol:
        ...
        if add_Smagorinsky:
            ...
            S = np.zeros((Nx, Ny, 2, 2))
            S[2:-2, 2:-2, 0, 0] = (u_old[2:-2, 3:-1] - u_old[2:-2, 1:-3]) / (2. * dx)
            S[2:-2, 2:-2, 0, 1] = 1. / 2. * (
                (u_old[3:-1, 2:-2] - u_old[1:-3, 2:-2]) / (2. * dy) \
                + (v_old[2:-2, 3:-1] - v_old[2:-2, 1:-3]) / (2. * dx)
            )
            S[2:-2, 2:-2, 1, 0] = S[2:-2, 2:-2, 0, 1]
            S[2:-2, 2:-2, 1, 1] = (v_old[3:-1, 2:-2] - v_old[1:-3, 2:-2]) / (2. * dy)
            nu_t = (c_filter * delta)**2 * np.sqrt(2. * np.sum(S**2, axis=(2, 3)))
        else:
            pass
        ...
        if n % 1000 == 0:
            ...
            levels = np.linspace(vmin, vmax, 32)
            ticks = np.linspace(vmin, vmax, 5)
            plt.contourf(X[2:-2, 2:-2], Y[2:-2, 2:-2], nu_t[2:-2, 2:-2], cmap="turbo", levels=levels, extend="both")
            plt.colorbar(ticks=ticks, extend="both", label=r"$\nu_{T}$")
            plt.xlim(0., Lx)
            plt.ylim(0., Ly)
            plt.xlabel(r"$x$")
            plt.ylabel(r"$y$")
            plt.title(f"Kinematic eddy viscosity ($\mathrm{{Re}}={Re}, t={t:.2f}$)")
            plt.tight_layout()
            plt.savefig(os.path.join(dir_res, f"nu_t.png"), dpi=300)
            plt.close()
